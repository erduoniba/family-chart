      ...
      if (window.personNodeHandler) {
        ...
        if (format === "png") {
          ...
          const params = {
            width: width,
            height: height,
            svgString: `data:image/svg+xml;charset=utf-8;base64,${btoa(
        ...
        downloadFile(
          svgString,
          format === "png" ? "family-tree.png" : "family-tree.svg",
          format === "png" ? "image/png" : "image/svg+xml"
        );
        callback({
          success: true,
          message: `${format.toUpperCase()} 已成功保存`,
        });
      }
    })
    .catch((error) => {
...
function prepareSvgForExport(svgElement) {
  ...
  const promises = Array.from(imageElements).map(async (img) => {
    ...
    try {
      ...
      const imgSrc = href.startsWith("http")
        ? href
